...
%define INT32_BASE                             0x7C00	; BIOS loaded us here, so its 'safe' I gues (matthijs)
...
%define STACK16                                (INT32_BASE - regs32_t_size)


section .text
	int32_beta: use32                          ; by Napalm
	_int32_beta:
...
; END OF MY HACK
		...
		mov  ecx, (int32_beta_end - reloc)     ; set copy size to our codes size
	...
	reloc: use32                               ; by Napalm
		...
		mov  ecx, regs32_t_size                ; set copy size to our struct size
...
;		pusha
; MY HACK
		pushad                                 ; save general purpose registers to 16bit stack
; END OF MY HACK
...
; END OF MY HACK
	...
	int32_beta_end:                                 ; end marker (so we can copy the code)
